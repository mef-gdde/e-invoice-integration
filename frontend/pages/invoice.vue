...
const {
    ...
    pagesList,
    fetch
} = useCrud<Document>('invoice', { take: 100 })
...
const active = ref('GENERATED')
const activeClass = (status: string) => {
    return {
        'inline-block p-4 text-blue-600 bg-gray-100 rounded-t-lg active dark:bg-gray-800 dark:text-blue-500': active.value === status,
        'inline-block p-4 rounded-t-lg hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300': active.value !== status
    }
}
watch(active, () => {
    fetch(1, { status: active.value })
})
...
const sendDocument = async (item: Document) => {
    await axios.post(`/invoice/${item.invoice_id}/send`)

    $swal.fire({
        title: 'Document was sent',
        text: `Document ${item.invoice_id} has been sent`,
        icon: 'success',
    })
...
<template>
    ...
    <div class="relative w-full p-4 text-center bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
        ...
        <li class="me-2">
            <a href="#" aria-current="page" :class="activeClass('GENERATED')" @click="active = 'GENERATED'">Generated</a>
        </li>
        <li class="me-2">
            <a href="#" :class="activeClass('SENT')" @click="active = 'SENT'">Sent</a>
        </li>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                >
                    ...
                    <td class="px-6 py-4">
                        {{ item.customer_company_name_en }}
                    </td>
                    <td class="px-6 py-4">
                        {{  priceFormat(item.tax_inclusive_amount) }}
                    </td>
                    <td class="px-6 py-4">
                        {{ item.issue_date }}
                    </td>
                    <td>
                        <button class="btn btn-dark mr-2" @click="viewDetail(item)">View
                            invoice</button>
                            <button class="btn btn-primary mr-2" @click="sendDocument(item)" v-if="active == 'GENERATED'">
                                Send Document    
                            </button>
                    </td>
                </tr>
